...
var noseX = 0, noseY = 0, gameStatus = "";
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press Start button to Start the game and Play ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(gameStatus == "start" && gameConfig.status==="start" && noseX != "" && noseY != "") {
    world_start.play();
...
function die(character){
    ...
    if(character == mario) mario_die.play();
}
